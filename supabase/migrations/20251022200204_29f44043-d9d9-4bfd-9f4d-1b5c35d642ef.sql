-- Phase 1: Database Schema Design for Multi-Role Review System (Revised)

-- 1. Create user roles enum (handle if exists)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.company_revisions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  product_id TEXT NOT NULL,
...
-- 11. RLS Policies for profiles (use DO blocks to handle existing policies)
DO $$ BEGIN
...
-- 20. Create indexes for performance (skip if exists)
CREATE INDEX IF NOT EXISTS idx_user_roles_user_id ON public.user_roles(user_id);
CREATE INDEX IF NOT EXISTS idx_user_roles_role ON public.user_roles(role);
CREATE INDEX IF NOT EXISTS idx_product_reviews_assigned_to ON public.product_reviews(assigned_to);
CREATE INDEX IF NOT EXISTS idx_product_reviews_product_id ON public.product_reviews(product_id);
CREATE INDEX IF NOT EXISTS idx_product_reviews_status ON public.product_reviews(status);
CREATE INDEX IF NOT EXISTS idx_product_reviews_deadline ON public.product_reviews(deadline);
CREATE INDEX IF NOT EXISTS idx_company_representatives_user_id ON public.company_representatives(user_id);
CREATE INDEX IF NOT EXISTS idx_company_representatives_company_id ON public.company_representatives(company_id);
CREATE INDEX IF NOT EXISTS idx_company_revisions_product_id ON public.company_revisions(product_id);
CREATE INDEX IF NOT EXISTS idx_review_comments_review_id ON public.review_comments(review_id);
CREATE INDEX IF NOT EXISTS idx_review_checklist_review_id ON public.review_checklist_items(review_id);